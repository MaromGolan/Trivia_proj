...
//function needed in 2.0.0;
LeaveRoom JsonRequestPacketDeserializer::deserializeLeaveRoomReq(const Buffer& buffer)
{
    json jsonContainer;
    toJson(buffer, jsonContainer);

    return { jsonContainer["username"].get<std::string>(), jsonContainer["room_name"].get<std::string>() };
...
RoomStatusRequest JsonRequestPacketDeserializer::deserializeRoomStateReq(const Buffer& buffer)
{
