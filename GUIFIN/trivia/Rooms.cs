...
{//TO ADD A START BUTTON TO FOR THE ADMIN THAT MOVES HIM TO THIS PAGE, AND A PROMPT THAT WHEN PRESSING YES MOVES THE USER TO THIS PAGE
    public partial class Rooms : Form
    {
        ...
        private void GameStarter()
        {
            if (thisuser == admin)
            {
                ...
                for (int i = 10; i > 0; i--)
                {
                    ...
                    Message res = JsonConvert.DeserializeObject<Message>(response);
                }
        ...
        private void gameEnded()
        {
            ...
            Message message = new Message(16,name);
        ...
        private void nextButton_Click(object sender, EventArgs e)
        {
            if (String.Equals(nextButton.Text, "next"))
            {
                ...
                numQuestion++;
                NetworkStream stream = usersocket.GetStream();
                Message message = new Message(15, questions[numQuestion].question + "," + correct + "," + (numQuestion - correct) + "," + totaltime/numQuestion + "," + name);
                string JSONmessage = JsonConvert.SerializeObject(message, Formatting.Indented);
                byte[] data = System.Text.Encoding.ASCII.GetBytes(JSONmessage);
                stream.Write(data, 0, data.Length);
            ...
            else
            {
                ...
                NetworkStream stream = usersocket.GetStream();
                Message message = new Message(11, name);
                string JSONmessage = JsonConvert.SerializeObject(message, Formatting.Indented);
                byte[] data = System.Text.Encoding.ASCII.GetBytes(JSONmessage);
                stream.Write(data, 0, data.Length);
                GameStarter();
        ...
        private void exit_Click(object sender, EventArgs e)
        {
            ...
            {
                NetworkStream stream = usersocket.GetStream();
                Message message = new Message(10,name);
                string JSONmessage = JsonConvert.SerializeObject(message, Formatting.Indented);
                byte[] data = System.Text.Encoding.ASCII.GetBytes(JSONmessage);
                stream.Write(data, 0, data.Length);
            }
            ...
            {
                NetworkStream stream = usersocket.GetStream();
                Message message = new Message(13,name);
                string JSONmessage = JsonConvert.SerializeObject(message, Formatting.Indented);
                byte[] data = System.Text.Encoding.ASCII.GetBytes(JSONmessage);
                stream.Write(data, 0, data.Length);
            }
